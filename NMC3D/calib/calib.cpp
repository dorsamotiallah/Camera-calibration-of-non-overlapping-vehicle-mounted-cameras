...
vector<MapPoint*> CalibC2C::SelectFrameDepthBalancedMatches(KeyFrame* pKF1, KeyFrame* pKF2, const vector<MapPoint*>& vpMatches)
{
    ...
    const Eigen::Matrix3f Rcw1 = pKF1->GetRotation();
    const Eigen::Vector3f tcw1 = pKF1->GetTranslation();
    ...
    for(size_t i = 0; i < vpMatches.size(); i++)
    {
        ...
        const Eigen::Vector3f Pc1 = Rcw1 * pMP1->GetWorldPos() + tcw1;
        const Eigen::Vector3f Pc2 = Rcw2 * pMP2->GetWorldPos() + tcw2;
        const float distance = 0.5f * (Pc1.norm() + Pc2.norm());
    ...
    const float minDistance = vDepthRefs.front().depth;
    const float maxDistance = vDepthRefs.back().depth;
    const float distanceRange = maxDistance - minDistance;

    if(!std::isfinite(distanceRange) || distanceRange <= 0.0f)
    {
        cout << "frame-to-frame distance-bin selection skipped: min " << minDistance
             << ", max " << maxDistance << endl;
        return vpMatches;
    }

    const float nearThreshold = minDistance + distanceRange / 3.0f;
    const float farThreshold = minDistance + 2.0f * distanceRange / 3.0f;

    std::array<vector<GlobalDepthMatchRef>, 3> vDepthGroups;
    for(const GlobalDepthMatchRef& ref : vDepthRefs)
    {
        if(ref.depth < nearThreshold)
            vDepthGroups[0].push_back(ref);
        else if(ref.depth < farThreshold)
            vDepthGroups[1].push_back(ref);
        else
            vDepthGroups[2].push_back(ref);
    }
    ...
    {
        cout << "frame-to-frame distance-bin selection skipped: near " << nNear
             << ", middle " << nMiddle
             << ", far " << nFar << endl;
        return vpMatches;
    }
    ...
    cout << "frame-to-frame distance-bin selection: valid " << nTotal
         << ", near " << nNear
         << ", middle " << nMiddle
         << ", far " << nFar
    ...
    cout << "frame-to-frame equal-width 3d distance bins: min " << minDistance
...
void CalibC2C::PrintGlobalDistanceDistribution(const vector<KeyFrame*>& vpKF1s, const vector<KeyFrame*>& vpKF2s,
    const vector<vector<MapPoint*>>& vvpMatches)
{
    vector<GlobalDepthMatchRef> vDistanceRefs;
    ...
    for(size_t idx = 0; idx < vvpMatches.size(); idx++)
    {
        ...
        for(size_t i = 0; i < vpMatches.size(); i++)
        {
            ...
            vDistanceRefs.push_back({idx, i, distance});
        }
    }

    if(vDistanceRefs.size() < 3)
    {
        cout << "global 3d distance distribution skipped: valid " << vDistanceRefs.size() << endl;
        return;
    }

    std::sort(vDistanceRefs.begin(), vDistanceRefs.end(), [](const GlobalDepthMatchRef& a, const GlobalDepthMatchRef& b) {
        return a.depth < b.depth;
    });

    const size_t nTotal = vDistanceRefs.size();
    const float minDistance = vDistanceRefs.front().depth;
    const float maxDistance = vDistanceRefs.back().depth;
    const float distanceRange = maxDistance - minDistance;

    if(!std::isfinite(distanceRange) || distanceRange <= 0.0f)
    {
        cout << "global equal-width 3d distance distribution skipped: min " << minDistance
             << ", max " << maxDistance << endl;
        return;
    }
    ...
    size_t nMiddle = 0;
    size_t nFar = 0;
    for(const GlobalDepthMatchRef& ref : vDistanceRefs)
    {
        if(ref.depth < nearThreshold)
            nNear++;
        else if(ref.depth < farThreshold)
    ...
    cout << "global equal-width 3d distance distribution: valid " << nTotal
         << ", near " << nNear
         << ", middle " << nMiddle
         << ", far " << nFar << endl;
    cout << "global equal-width 3d distance thresholds: min " << minDistance
         << ", near < " << nearThreshold
         << ", middle < " << farThreshold
         << ", max " << maxDistance << endl;
    cout << "global 3d distance stats: min " << PercentileDepth(vDistanceRefs, 0.0f)
         << ", median " << PercentileDepth(vDistanceRefs, 0.50f)
         << ", max " << PercentileDepth(vDistanceRefs, 1.0f) << endl;
}
