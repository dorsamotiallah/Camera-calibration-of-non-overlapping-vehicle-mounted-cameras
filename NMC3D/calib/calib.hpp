...
public:
    ...
    void clearSaveAtlasPath() { mStrSaveAtlasToFile.clear(); }
};
...
private:
    ...
    const bool bUseDepthBalancedSelection = true;
    const int minDepthBalancedMatches = 15;
...
public:
    ...
    vector<MapPoint*> SelectFrameDepthBalancedMatches(KeyFrame* pKF1, KeyFrame* pKF2, const vector<MapPoint*>& vpMatches);

    void PrintGlobalDistanceDistribution(const vector<KeyFrame*>& vpKF1s, const vector<KeyFrame*>& vpKF2s,
        const vector<vector<MapPoint*>>& vvpMatches);

    vector<vector<MapPoint*>> SelectGlobalDepthBalancedMatches(const vector<KeyFrame*>& vpKF1s, const vector<KeyFrame*>& vpKF2s,
        const vector<vector<MapPoint*>>& vvpMatches);
